...
def showusers():
    ...
    with tab1:
        ...
        if not users:
            ...
            # WICHTIG: Jeder User bekommt einen Expander
            for u in users:
                with st.expander(f"👤 {u.get('name')} ({u.get('email')})"):
                    ...
                    with col2:
                        ...
                        # NEU: PASSWORT ZURÜCKSETZEN - HIER IST DER BUTTON!
                    ...
                    if st.session_state.get(f'show_reset_{u["id"]}', False):
                        ...
                        with st.form(f"reset_form_{u['id']}"):
                            ...
                            new_pw = st.text_input("Neues Passwort", type="password", help="Mindestens 6 Zeichen", key=f"pw1_{u['id']}")
                            new_pw2 = st.text_input("Passwort bestätigen", type="password", key=f"pw2_{u['id']}")
