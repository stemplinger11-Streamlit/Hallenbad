"""
Wasserwacht Dienstplan+ v7.3 - Final Production
Alle Bugs behoben | E-Mail/SMS funktionieren | Dashboard funktioniert
"""
...
class WasserwachtDB:
    ...
    def get_stats(self):
        """FIX: Robuste Statistiken - komplett neu"""
        try:
            ...
            # Future Bookings - EINFACHE QUERY
            future_bookings = []
            try:
    ...
    def archive_old(self):
        ...
        try:
            months = 12  # Nach 12 Monaten archivieren
...
# ===== ENDE TEIL 1 =====
# ===== EMAIL & SMS CLASSES (AUS ALTER FUNKTIONIERENDER VERSION) =====
class Mailer:
    def __init__(self):
        if hasattr(st,'secrets'):
            ...
            self.from_name = "Dienstplan"
    
    def send(self,to,subject,body,attachments=None):
        """FIX: Syntax aus alter funktionierender Version"""
        if not self.user or not self.pw:
            print("❌ E-Mail: Keine Credentials")
            return False
        ...
        try:
            ...
            print(f"❌ SMTP Auth Fehler: {e}")
            st.error("🔴 E-Mail-Auth Fehler! Prüfe SMTP_PASSWORD in Secrets (Gmail App-Passwort!)")
            return False
        except Exception as e:
    ...
    def booking_confirmation(self,user_email,user_name,slot_date,slot_time):
        ...
        <p>{template.format(name=user_name,date=fmt_de(slot_date),time=slot_time)}</p>
        <hr>
        ...
        return self.send(user_email,f"✅ Buchungsbestätigung {fmt_de(slot_date)}",body)
    ...
    def backup_email(self,backup_zip):
        ...
        try:
            ...
            <hr>
            <p style='color:{COLORS['grau_dunkel']};font-size:0.9rem'>
            Dies ist ein automatisches Backup des Dienstplan-Systems.<br>
            ...
            # Backup-E-Mail-Adressen aus Secrets
            backup_emails = []
            if hasattr(st,'secrets'):
                try:
...
class TwilioSMS:
    ...
    def send(self,to,message):
        """FIX: Syntax aus alter funktionierender Version"""
        if not self.enabled:
            print("❌ SMS: Nicht aktiviert (ENABLE_SMS_REMINDER=false)")
            return False
        
        if not self.sid or not self.token or not self.phone:
            print("❌ SMS: Keine Credentials")
            return False
        
        try:
            # Format Nummer (füge +49 hinzu wenn nötig)
    ...
    def booking_confirmation(self,user_phone,user_name,slot_date,slot_time):
        ...
        template = ww_db.get_setting('sms_booking_template',
            'Wasserwacht: Buchung bestätigt! {date} {time}. Wir freuen uns!')
...
def check_free_slots_alarm():
    try:
        ...
        print(f"Free slots alarm error: {e}")
...
def show_home():
    ...
    # FIX: Buchungen SOFORT laden und anzeigen
    bookings = ww_db.get_week_bookings(cws.strftime("%Y-%m-%d"))
    
    # DEBUG für Admin (zeigt was geladen wurde)
    if user.get('role') == 'admin':
        with st.expander("🔧 Debug (Admin)"):
            st.write(f"**Geladene Buchungen:** {len(bookings)}")
            if bookings:
                for b in bookings[:3]:
                    st.json({'datum': b['slot_date'], 'zeit': b['slot_time'], 'user': b['user_name']})
    
    # Erstelle Mapping für schnellen Zugriff
    booking_map = {}
    for b in bookings:
        key = (b['slot_date'], b['slot_time'])
        booking_map[key] = b
    ...
    for slot in WEEKLY_SLOTS:
        ...
        if blocked:
            ...
            status_text = f"<span style='color:{COLORS['grau_dunkel']}'>🚫 {block_reason(sd) or ''}</span>"
        elif booking:
            # ORANGE HINTERGRUND + NAME ANZEIGEN!
            bg_color = f"{COLORS['orange']}30"
            border_color = COLORS['orange']
            status_text = f"<span style='color:{COLORS['orange']};font-weight:600'>✅ Bereits von: <b>{booking['user_name']}</b> gebucht</span>"
        else:
            bg_color = f"{COLORS['blau']}10"
            border_color = COLORS['blau_hell']
            status_text = "<span style='color:{COLORS['erfolg']}'>📅 Slot verfügbar</span>"
        ...
        # Buttons
        if blocked:
            pass  # Keine Buttons bei Blockierung
        elif booking:
            # Slot ist gebucht
...
def show_month_calendar():
    ...
    # NEU: Kalender klickbar machen
    user = st.session_state.user
    st.info("💡 Klicke auf einen Tag zum Buchen")
        ...
        for day_idx, day in enumerate(week):
            with cols[day_idx]:
                if day==0:
                    ...
                    # Tag-Button
                    bg = COLORS['rot'] if is_today else (COLORS['grau_mittel'] if blocked else COLORS['erfolg'] if day_bookings else COLORS['grau_hell'])
                    st.markdown(f"<div style='background:{bg}20;padding:0.5rem;border-radius:8px;text-align:center'><b>{day}</b></div>",unsafe_allow_html=True)
                    
                    # Slots für diesen Tag
                    for slot in WEEKLY_SLOTS:
                        ws = week_start(datetime.strptime(date_str,"%Y-%m-%d"))
                        slot_d = slot_date(ws,slot['day'])
                        if slot_d == date_str:
                            slot_time = f"{slot['start']}-{slot['end']}"
                            existing = ww_db.get_booking(date_str,slot_time)
                            
                            if blocked:
                                st.caption(f"🚫 {slot['start']}")
                            elif existing:
                                st.caption(f"✅ {slot['start']}")
                                st.caption(f"{existing['user_name'][:8]}")
                            else:
                                if st.button(f"📅 {slot['start']}",key=f"cal_{date_str}_{slot['id']}",use_container_width=True):
                                    success,msg = ww_db.create_booking(date_str,slot_time,user['email'],user['name'],user.get('phone',''))
                                    if success:
                                        mailer.booking_confirmation(user['email'],user['name'],date_str,slot_time)
                                        st.success("Gebucht!")
                                        st.rerun()
                                    else:
                                        st.error(msg)
        ...
        with col1:
            if st.button("📧 Test-E-Mail",use_container_width=True):
                if mailer.test_email(user['email']):
                    ...
                    st.error("❌ E-Mail-Fehler! Prüfe SMTP-Settings in Secrets.")
        with col2:
            if st.button("📱 Test-SMS",use_container_width=True):
                if not user.get('phone'):
                    ...
                    st.error("❌ SMS-Fehler! Prüfe Twilio-Settings in Secrets.")
...
def show_dashboard():
    ...
    try:
        ...
        if user_stats:
            # Zähle Schichten pro User
            from collections import Counter
            user_counts = Counter([u['user_name'] for u in user_stats])
            ...
            with col1:
                # Balkendiagramm
                fig = px.bar(df_scoreboard.head(10), x='Name', y='Schichten', 
                             title='Top 10 User nach Schichten',
                             color='Schichten',
            ...
            with col2:
                ...
                for idx, row in df_scoreboard.iterrows():
                    if idx == 0:
    ...
    st.divider()
    st.subheader("📈 Buchungstrend (letzte 4 Wochen)")
    try:
        trend_data = []
        for i in range(28):
            check_date = (datetime.now()-timedelta(days=27-i)).strftime("%Y-%m-%d")
            count = 0
            for doc in ww_db.db.collection('bookings').where('slot_date','==',check_date)\
                    .where('status','==','confirmed').stream():
                count += 1
            trend_data.append({'Datum':check_date,'Buchungen':count})
        
        if trend_data:
            df = pd.DataFrame(trend_data)
            fig = px.line(df,x='Datum',y='Buchungen',title='Buchungen pro Tag')
            st.plotly_chart(fig,use_container_width=True)
    except Exception as e:
...
def show_settings():
    ...
    with tab2:
        ...
        sms_booking = ww_db.get_setting('sms_booking_template','Wasserwacht: Buchung bestätigt! {date} {time}')
