...
# ===== PAGE CONFIG (MUSS GANZ AM ANFANG!) =====
st.set_page_config(
...
class WasserwachtDB:
    ...
    def _init_admin(self):
        """Erstellt Initial-Admin falls nicht vorhanden"""
        if hasattr(st, 'secrets'):
            ...
            if not existing_user:
                ...
                try:
                    ...
                    print(f"✅ Admin erstellt: {email}")  # Debug-Log
                except Exception as e:
                    print(f"❌ Admin-Erstellung fehlgeschlagen: {e}")
            else:
                print(f"ℹ️ Admin existiert bereits: {email}")  # Debug-Log
...
# ===== MAIN APP =====
def main():
    # st.set_page_config ist jetzt am Anfang der Datei!
            ...
            if st.button("Login", type="primary", use_container_width=True):
                if not email or not pw:
                    ...
                    if success:
                        ...
                        st.info("💡 Tipp: Prüfe E-Mail und Passwort. Bei Problemen wende dich an den Admin.")
        
        with col2:
            if st.button("🔄 Reset", use_container_width=True):
                st.rerun()
        
        # Debug-Info (nur während Entwicklung, später entfernen)
        with st.expander("🔧 Debug-Info (Admin-Daten)"):
            if hasattr(st, 'secrets'):
                st.code(f"Admin E-Mail: {st.secrets.get('ADMIN_EMAIL', 'NICHT GESETZT')}")
                st.info("Das Admin-Passwort steht in den Secrets (ADMIN_PASSWORD)")
            else:
                st.warning("Keine Secrets konfiguriert!")
    
    with tab2:
        st.subheader("Neuen Account erstellen")
        
        with st.form("register_form"):
            reg_name = st.text_input("Vollständiger Name *", help="z.B. Max Mustermann")
            reg_email = st.text_input("E-Mail *", help="Deine gültige E-Mail-Adresse")
            reg_phone = st.text_input("Telefonnummer", help="Für SMS-Reminder (optional)")
            reg_pw = st.text_input("Passwort *", type="password", help="Min. 8 Zeichen")
            reg_pw_confirm = st.text_input("Passwort bestätigen *", type="password")
            
            st.markdown("**Mit der Registrierung akzeptierst du:**")
            accept_terms = st.checkbox("Datenschutzerklärung & Nutzungsbedingungen", value=False)
            
            submitted = st.form_submit_button("📝 Jetzt registrieren", type="primary", use_container_width=True)
            ...
            if submitted:
                ...
                if not reg_name or not reg_email or not reg_pw or not reg_pw_confirm:
                    ...
                    success, msg = ww_db.create_user(reg_email, reg_name, reg_phone, reg_pw, role='user')
                    if success:
                        st.success(f"✅ {msg}")
                        st.success("Du kannst dich jetzt im Login-Tab anmelden!")
                        st.balloons()
                        # Optional: E-Mail-Benachrichtigung an Admin
                        try:
                            mailer.send(
                                st.secrets.get("ADMIN_EMAIL_RECEIVER", ""),
                                "Neue Registrierung",
                                f"<p>Neuer User registriert:</p><ul><li>Name: {reg_name}</li><li>E-Mail: {reg_email}</li></ul>"
                            )
                        except:
                            pass
                    else:
                        st.error(f"❌ Registrierung fehlgeschlagen: {msg}")
        
        st.divider()
        st.info("💡 **Hinweis:** Nach der Registrierung kannst du dich sofort anmelden und Schichten buchen!")
