...
class Mailer:
    ...
    def send(self,to,subject,body,attachments=None):
        """FIX: Code aus alter funktionierender Version"""
        if not self.user or not self.pw:
            print("E-Mail: Keine Credentials")
            return False
        try:
            msg = MIMEMultipart()
            msg['From'] = email.utils.formataddr(("Wasserwacht Dienstplan", self.user))
            msg['To'] = to
            msg['Subject'] = subject
            ...
            st.error(f"E-Mail-Auth Fehler: Prüfe SMTP_USER und SMTP_PASSWORD in Secrets!")
            return False
    ...
    def test_email(self,to):
        ...
        <p>Zeitpunkt: {datetime.now(TZ).strftime('%d.%m.%Y %H:%M:%S')}</p>
        </body></html>"""
    ...
    def backup_email(self,backup_zip):
        try:
            ...
            <p><b>Zeitpunkt:</b> {datetime.now(TZ).strftime('%d.%m.%Y %H:%M')}</p>
...
def show_home():
    ...
    # FIX: Buchungen laden mit Logging
    ...
    if user.get('role') == 'admin':
        with st.expander("🔧 Debug-Info (Admin)"):
            ...
            if bookings:
                st.json([{'date':b['slot_date'],'time':b['slot_time'],'user':b['user_name']} for b in bookings[:3]])
    ...
    for slot in WEEKLY_SLOTS:
        ...
        # FIX: Name anzeigen!
        booked_info = f"<span style='color:{COLORS['orange']};font-weight:600'>✅ Gebucht von: {booking['user_name']}</span>" if booking else ""
        ...
        card_html = f'''<div style="background:{bg_color};border:2px solid {border_color};border-radius:12px;
            ...
            <div style="display:flex;justify-content:space-between;align-items:center">
                ...
                <div style="color:{COLORS['grau_dunkel']};font-size:0.9rem">{slot_time_str}</div></div>
