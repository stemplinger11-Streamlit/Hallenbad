...
class Mailer:
    def __init__(self):
        ...
        if hasattr(st, 'secrets'):
            print(f"DEBUG: st.secrets = {dict(st.secrets)}")
            print(f"DEBUG: SMTP_USER in secrets? {'SMTP_USER' in st.secrets}")
            print(f"DEBUG: GMAIL_USER in secrets? {'GMAIL_USER' in st.secrets}")
        else:
    ...
    def send_password_reset(self, user_email, user_name, new_password):
        """Passwort-Reset E-Mail mit neuem Passwort"""
        body = f"""<html><body style='font-family:Arial,sans-serif'>
...
def show_users():
    ...
    with tab1:
        ...
        for u in users:
            # NEU: Expander für jeden User (übersichtlicher)
            with st.expander(f"👤 {u.get('name')} ({u.get('email')})"):
                ...
                with col2:
                    # Löschen (nur wenn nicht eigener Account)
                ...
                if st.session_state.get(f'show_reset_{u["id"]}', False):
                    ...
                    with st.form(f"reset_form_{u['id']}"):
                        ...
                        confirm = st.checkbox(f"✅ Ja, Passwort für **{u.get('name')}** JETZT zurücksetzen", key=f"confirm_{u['id']}")
                
                        col_a, col_b = st.columns(2)
                
                        with col_a:
                            submit = st.form_submit_button("🔐 Zurücksetzen", use_container_width=True, type="primary")
                        with col_b:
                        ...
                        if submit:
                            if not new_pw or not new_pw2:
                                ...
                                admin_name = st.session_state.user.get('name')
                                ...
                                # E-Mail senden
                                if mailer.send_password_reset(u.get('email'), u.get('name'), new_pw):
                                    ...
                                    st.error("❌ Passwort wurde gesetzt, aber E-Mail konnte nicht versendet werden!")
    ...
    # NEU: Tab 3 - Für User buchen
    with tab3:
        st.subheader("🎯 Schicht für User buchen")
        
        users = ww_db.get_all_users()
        if not users:
            st.warning("Keine User vorhanden")
            return
        
        # User auswählen
        user_options = [(u['name'], u['email'], u.get('phone', '')) for u in users]
        ...
        # Woche auswählen
        selected_date = st.date_input("Woche auswählen", value=datetime.now())
        ws = week_start(selected_date)
        
        st.info(f"Buchung für: **{selected_user[0]}** | KW {ws.isocalendar()[1]}, {ws.year}")
        
        # Slot auswählen
        slot_options = []
        for slot in WEEKLY_SLOTS:
            sd = slot_date(ws, slot['day'])
            slot_time = f"{slot['start']}-{slot['end']}"
            
            # Prüfe ob bereits gebucht
            existing = ww_db.get_booking(sd, slot_time)
            if existing:
                label = f"❌ {slot['day_name']} {fmt_de(sd)} {slot_time} (Gebucht von {existing['user_name']})"
                slot_options.append((label, None, None, True))
            elif is_blocked(sd):
                label = f"🚫 {slot['day_name']} {fmt_de(sd)} {slot_time} ({block_reason(sd)})"
                slot_options.append((label, None, None, True))
            else:
                label = f"✅ {slot['day_name']} {fmt_de(sd)} {slot_time}"
                slot_options.append((label, sd, slot_time, False))
        
        # Nur verfügbare Slots anzeigen
        available_slots = [s for s in slot_options if not s[3]]
        
        if not available_slots:
            st.warning("Keine freien Slots in dieser Woche")
        else:
            ...
            if st.button("🎯 Für User buchen", type="primary", use_container_width=True):
                slot_date_str = selected_slot[1]
                slot_time_str = selected_slot[2]
                
                success, msg = ww_db.create_booking(
                    slot_date_str,
                    slot_time_str,
                    selected_user[1],  # email
                    selected_user[0],  # name
                    selected_user[2]   # phone
                )
                
                if success:
                    st.success(f"✅ Gebucht für {selected_user[0]}!")
                    ...
                    if selected_user[2]:
                        u = ww_db.get_user(selected_user[1])
                        if u and u.get('sms_booking_confirmation'):
        ...
        # Alle gebuchten/blockierten anzeigen
        st.divider()
        st.caption("**Alle Slots dieser Woche:**")
        for s in slot_options:
            if s[3]:  # gebucht/blockiert
                st.caption(s[0])
